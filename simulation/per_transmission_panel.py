...
class PerTransmissionPanel(Panel):
    ...
    def _effective_data_sequence(self, seq):
        ...
        # Explicit one-shot DATA rules override the continuous fallback for this
        # transmission only. The continuous rule resumes on a later DATA
        # transmission whose configured mode is none.
        ...
        self.log(
            f"SEQ {self.seq_i} uses continuous random_probability="
            f"{self.continuous_probability_loss['probability'] * 100:g}% for DATA"
        )
        return effective
    ...
    def _accept_nack(self, sender, data64, frame):
        ...
        if state is None:
            ...
            self.log(
                f"warning: NACK transmission page count changed for "
                f"msg=0x{frame.message_id:08X}; starting a new transmission sequence"
            )
    ...
    def accept_frame(self, sender, data64):
        ...
        if frame.frame_type == FrameType.END:
            # END is a separate radio/protocol transmission. If it closes a DATA
            # burst, finish that DATA transmission's metadata first, then consume
            # the next scenario specifically for END.
            if (
        ...
        # Defensive fallback for any future control frame type: it still counts
        # as one transmission and therefore consumes one scenario.
